...
def _log(t, u, p, f, vol):
    import os
    path = os.environ.get("SWAN_LOG_CSV")
    if not path:
        return
    new = not os.path.exists(path)
    with open(path, "a") as fh:
        if new:
            fh.write("time,u_norm,p_norm,solid_force_norm,volume\n")
        fh.write(f"{t:.8g},{u:.8g},{p:.8g},{f:.8g},{vol:.8g}\n")
...
for step in range(config['num_steps']):
    ...
    if time_manager.should_output(step):
        ...
        if MPI.COMM_WORLD.rank == 0:
            ...
            _log(current_time, u_L2, p_L2, F_L2, volume)
